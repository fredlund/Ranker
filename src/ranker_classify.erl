...
classiphy(Name,N,Students,Size,Gen,Test) ->
  io:format("Students are~n~p~n",[Students]),
  classes(Name,1,Students,Size,N,[],Gen,Test).

classes(Name,I,Students,Size,N,Classes,Gen,Test) ->
  AllPairs =
    lists:usort
      (lists:flatmap
	 (fun (Class) ->
  ...
  %%("~nall loaded size:~p~n~p~n",[length(code:all_loaded()),code:all_loaded()]),
	...
	ResultP = #result{failures=Ps,successes=Students--Ps,testcase=TestCase},
	if
	  Ps==[] ->
	...
	case covered(Classes,ResultP) of
	  true ->
	    io:format
	      ("*** Warning: failing students~n~s~nARE COVERED BY~n tests~n~s~n",
	...
	[io:format("*** Warning: values differ??? ps:~n~p~n qs:~n~p~n",[Ps,Qs])
	 || Ps=/=Qs],
	...
	file:write_file
	  (Name++"_classes_"++integer_to_list(I)++".bin",
	   term_to_binary({classified,Students,NewClasses})),
	...
	classes(Name,I+1,Students,Size,N,NewClasses,Gen,Test)
    end.
...
print_class(Class) ->
  lists:foldr(fun (Student,Acc) -> Student++", "++Acc end, "", Class#result.failures).

prop_complete(N,Classes,Gen,Students,Test) ->
    numtests(
      N,
      ?ALWAYS(3,
	      ?FORALL(TestCase,
		      Gen,
		      %%noshrink(Gen),
		      pcovered(Classes,Test,Students,TestCase)))).
...
pcovered(Classes,Test,Students,TestCase) ->
  Ps = lists:usort(Test(TestCase)),
  Result = #result{failures=Ps,successes=Students--Ps,testcase=TestCase},
...
shrink_measure(PreOldMeasure,Result,{false,CovMeasure}) ->
  NumFailures =
...
calculate_graph(Students,PreClasses) ->
  Classes =
...
graph(Name,Render,Students,Classes,Options) ->
  {States,Edges} = calculate_graph(Students,Classes),
...
statistics(Name,States,Edges) ->
  io:format("~nStates are~n~p~n",[States]),
  io:format("Edges are~n~p~n",[Edges]),
  Starters =
    lists:filter
      (fun (State) -> 
	   not(lists:any(fun ({_,Q,_}) -> Q==State end, Edges))
       end,States),
  io:format("Starters are~n~p~n",[Starters]),
  NonStarters = 
    States--Starters,
  io:format("Nonstarters are~n~p~n",[NonStarters]),
  StarterPaths =
    lists:map
